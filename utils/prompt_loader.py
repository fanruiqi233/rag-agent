...
def load_system_prompts():
    try:
        ...
        logger.error(f"[load_system_prompts]在yaml配置项中没有main_prompt配置项")
        raise e
    try:
...
def load_rag_prompts():
    try:
        ...
        logger.error(f"[load_rag_prompts]在yaml配置项中没有rag_prompt配置项")
        raise e
    try:
...
def load_report_prompts():
    try:
        ...
        logger.error(f"[load_system_prompts]在yaml配置项中没有report_prompt配置项")
        raise e
    try:
        return open(report_prompts_path,"r",encoding="utf-8").read()

    except Exception as e:
        logger.error(f"[load_report_prompt]解析报告生成出错,{str(e)}")
